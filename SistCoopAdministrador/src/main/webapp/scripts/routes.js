  ...
  return app.config(['$httpProvider', '$stateProvider', '$urlRouterProvider', 'RestangularProvider', 'localStorageServiceProvider',
    function ($httpProvider, $stateProvider, $urlRouterProvider, RestangularProvider, localStorageServiceProvider) {
      ...
      $stateProvider
          ...
          views: {
            "viewMenu": {
              controller: function ($scope) {
                $scope.menus = [
                  {
                    'name': 'Reportes', submenus: [
                    {'name': 'Debe/Haber', 'state': 'app.reportes.debeHaber'}
                  ]
                  }
                ];
              }
            ...
            "viewContent": {
              template: '<div ui-view style="min-height: 472px;"><h4 class="text-center" style="font-weight: bold; color: blue;">REPORTES</h4></div>'
            }
          }
        })
        ...
        .state('app.reportes.debeHaber', {
          url: "/debeHaber",
          templateUrl: "views/administrador/reportes/debeHaber.html"
        });
    }
