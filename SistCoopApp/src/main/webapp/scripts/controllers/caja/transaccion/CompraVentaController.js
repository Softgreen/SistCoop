  ...
  controllers.controller('CompraVentaController', ["$scope", "$state", "$window", "$timeout", "$filter", "$modal", "focus", "CuentaBancariaService", "SessionService", "MonedaService", "PersonaNaturalService", "TasaInteresService",
    function ($scope, $state, $window, $timeout, $filter, $modal, focus, CuentaBancariaService, SessionService, MonedaService, PersonaNaturalService, TasaInteresService) {
      ...
      $scope.$watch("view.idMonedaRecibida", function (newVal, oldVal) {
        if (newVal !== oldVal) {
          if (!angular.isUndefined($scope.view.idMonedaRecibida) && !angular.isUndefined($scope.view.idMonedaEntregada)) {
      ...
      //nueva funcionalidad
      $scope.operacionSelected = '';
      $scope.$watch('operacionSelected', function(newValue){
        if(newValue){
          switch(newValue) {
            case 'compraDolares':
              $scope.view.tipoOperacion = $scope.combo.tipoOperacion[0];//compra
              $scope.view.idMonedaRecibida = 0;//dolares
              $scope.view.idMonedaEntregada = 1;//soles
              break;
            case 'ventaDolares':
              $scope.view.tipoOperacion = $scope.combo.tipoOperacion[1];//venta
              $scope.view.idMonedaRecibida = 1;//soles
              $scope.view.idMonedaEntregada = 0;//dolares
              break;
            case 'compraEuros':
              $scope.view.tipoOperacion = $scope.combo.tipoOperacion[0];//compra
              $scope.view.idMonedaRecibida = 2;//euros
              $scope.view.idMonedaEntregada = 1;//soles
              break;
            case 'ventaEuros':
              $scope.view.tipoOperacion = $scope.combo.tipoOperacion[1];//venta
              $scope.view.idMonedaRecibida = 1;//soles
              $scope.view.idMonedaEntregada = 2;//euros
              break;
            default:
              return;
          }
