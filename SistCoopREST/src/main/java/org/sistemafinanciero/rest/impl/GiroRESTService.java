...
public class GiroRESTService implements GiroREST {
	...
	@Override
	public Response getGirosEnviados(BigInteger idAgencia, String filterText,
			Integer offset, Integer limit) {
		List<Giro> list = giroServiceNT.getGirosEnviados(idAgencia, filterText, offset, limit);
		Response response = Response.status(Response.Status.OK).entity(list).build();
		return response;
	}

	@Override
	public Response getGirosRecibidos(BigInteger idAgencia, String filterText, Integer offset, Integer limit) {
		List<Giro> list = giroServiceNT.getGirosRecibidos(idAgencia,filterText, offset, limit);
		Response response = Response.status(Response.Status.OK).entity(list).build();
		return response;
	}
