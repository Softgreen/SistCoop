...
public class CajaSessionServiceBeanTS implements CajaSessionServiceTS {
	...
	public BigInteger abrirCaja() throws RollbackFailureException {
		...
		if (trabajador == null)
			throw new RollbackFailureException(
					"No se encontró un trabajador para la caja");
		...
		for (BovedaCaja bovedaCaja : bovedaCajas) {
			...
			if (!boveda.getAbierto())
				throw new RollbackFailureException(
						"Debe de abrir las bovedas asociadas a la caja("
		...
		try {
			...
			Set<ConstraintViolation<Caja>> violationsCaja = validator
					.validate(caja);
			if (!violationsCaja.isEmpty()) {
				throw new ConstraintViolationException(
						new HashSet<ConstraintViolation<?>>(violationsCaja));
			} else {
				cajaDAO.update(caja);
			}
			...
			historialCajaNew.setTrabajador(trabajador.getPersonaNatural()
					.getApellidoPaterno()
					+ " "
			...
			if (historialCajaOld != null) {
				Set<DetalleHistorialCaja> detalleHistorialCajas = historialCajaOld
						.getDetalleHistorialCajas();
				...
				for (BovedaCaja bovedaCaja : bovedaCajas) {
					...
					List<MonedaDenominacion> denominaciones = monedaService
							.getDenominaciones(moneda.getIdMoneda());
					for (MonedaDenominacion monedaDenominacion : denominaciones) {
						...
						detalleHistorialCaja.setHistorialCaja(historialCajaNew);
						detalleHistorialCaja
								.setMonedaDenominacion(monedaDenominacion);
						...
						if (!violationsHistorialDetalle.isEmpty()) {
							...
							detalleHistorialCajaDAO
									.create(detalleHistorialCaja);
						}
					}
				}
			}
	...
	@Override
	public BigInteger cerrarCaja(Set<GenericMonedaDetalle> detalleCaja)
		...
		try {
			...
			Set<ConstraintViolation<HistorialCaja>> violationsHistorial = validator
					.validate(historialCaja);
			if (!violationsHistorial.isEmpty()) {
				throw new ConstraintViolationException(
						new HashSet<ConstraintViolation<?>>(violationsHistorial));
			} else {
				...
				for (BovedaCaja bovedaCaja : bovedas) {
					...
					for (GenericMonedaDetalle detalle : detalleCaja) {
						...
						if (monedaBoveda.equals(monedaUsuario)) {
							Set<DetalleHistorialCaja> detHistCaja = historialCaja
									.getDetalleHistorialCajas();
							Set<GenericDetalle> genDet = detalle.getDetalle();
							for (DetalleHistorialCaja dhc : detHistCaja) {
								MonedaDenominacion monedaDenom = dhc
										.getMonedaDenominacion();
								BigDecimal valorMonedaDenom = monedaDenom
										.getValor();
								for (GenericDetalle genericDetalle : genDet) {
									if (genericDetalle.getValor().compareTo(
											valorMonedaDenom) == 0
											&& monedaDenom.getMoneda().equals(
													monedaUsuario)) {
										dhc.setCantidad(genericDetalle
												.getCantidad());
	...
	public BigInteger crearAporte(BigInteger idSocio, BigDecimal monto,
		...
		if (cuentaAporte == null)
			throw new RollbackFailureException(
					"Socio no tiene cuenta de aportes");
	...
	public BigInteger retiroCuentaAporte(BigInteger idSocio)
		...
		BigDecimal saldoDisponible = cuentaAporte.getSaldo().add(
				montoTransaccion);
		cuentaAporte.setSaldo(saldoDisponible);
		cuentaAporteDAO.update(cuentaAporte);
		...
		transaccionCuentaAporte.setSaldoDisponible(saldoDisponible);
		transaccionCuentaAporte
				.setTipoTransaccion(Tipotransaccionbancaria.RETIRO);

		transaccionCuentaAporteDAO.create(transaccionCuentaAporte);

		// actualizar saldo caja
		this.actualizarSaldoCaja(montoTransaccion, cuentaAporte.getMoneda()
				.getIdMoneda());
	...
	@Override
	public BigInteger crearCompraVenta(
			Tipotransaccioncompraventa tipoTransaccion,
			BigInteger idMonedaRecibido, BigInteger idMonedaEntregado,
			BigDecimal montoRecibido, BigDecimal montoEntregado,
			BigDecimal tasaCambio, String referencia)
			throws RollbackFailureException {
		...
		transaccionCompraVenta.setObservacion("Doc:"
				+ natural.getTipoDocumento().getAbreviatura() + "/"
				+ natural.getNumeroDocumento() + "Trabajador:"
				+ natural.getApellidoPaterno() + " "
				+ natural.getApellidoMaterno() + "," + natural.getNombres());
		...
		actualizarSaldoCaja(montoEntregado.abs().negate(),
				monedaEntregada.getIdMoneda());
	...
	public BigInteger crearTransferenciaBancaria(String numeroCuentaOrigen,
		...
		// obteniendo saldo disponible de cuenta
		BigDecimal saldoDisponibleOrigen = cuentaBancariaOrigen.getSaldo()
				.subtract(monto);
		BigDecimal saldoDisponibleDestino = cuentaBancariaDestino.getSaldo()
				.add(monto);
		...
		if (cuentaBancariaDestino.getTipoCuentaBancaria().equals(
				TipoCuentaBancaria.PLAZO_FIJO)) {
		...
		transferenciaBancaria.setObservacion("Doc:"
				+ natural.getTipoDocumento().getAbreviatura() + "/"
				+ natural.getNumeroDocumento() + "Trabajador:"
				+ natural.getApellidoPaterno() + " "
				+ natural.getApellidoMaterno() + "," + natural.getNombres());
	...
	@Override
	@AllowedTo(Permission.ABIERTO)
	public void extornarTransaccion(BigInteger idTransaccion)
			throws RollbackFailureException {
		HistorialTransaccionCaja transaccion = historialTransaccionCajaDAO
				.find(idTransaccion);
	...
	private void extornarTransaccionBancaria(BigInteger idTransaccion)
			throws RollbackFailureException {
		TransaccionBancaria transaccionBancaria = transaccionBancariaDAO
				.find(idTransaccion);
		if (transaccionBancaria.getTipoTransaccion().equals(
				Tipotransaccionbancaria.DEPOSITO))
			extornarCuentaBancariaDeposito(transaccionBancaria);
		else
			extornarCuentaBancariaRetiro(transaccionBancaria);
	}

	private void extornarCuentaBancariaDeposito(
			TransaccionBancaria transaccionBancaria)
			throws RollbackFailureException {
		// condicionar si la caja tiene saldo para devolver
		CuentaBancaria cuentaBancaria = cuentaBancariaDAO
				.find(transaccionBancaria.getCuentaBancaria()
						.getIdCuentaBancaria());
		...
		if (transaccionBancaria.getEstado() == true
				&& cuentaBancaria.getEstado().equals(
						EstadoCuentaBancaria.ACTIVO)
				&& transaccionBancaria.getHistorialCaja().getIdHistorialCaja() == historialCajaActivo
						.getIdHistorialCaja()) {
			if (cuentaBancaria.getSaldo().compareTo(
					transaccionBancaria.getMonto()) != -1) {
				Caja caja = this.getCaja();
				...
				if (saldoActualBovedaCaja.compareTo(transaccionBancaria
						.getMonto()) != -1) {
					cuentaBancaria.setSaldo(cuentaBancaria.getSaldo().abs()
							.subtract(transaccionBancaria.getMonto().abs()));
					actualizarSaldoCaja(transaccionBancaria.getMonto().abs()
							.negate(), transaccionBancaria.getMoneda()
							.getIdMoneda());
	...
	private void extornarCuentaBancariaRetiro(
			TransaccionBancaria transaccionBancaria)
			throws RollbackFailureException {
		CuentaBancaria cuentaBancaria = cuentaBancariaDAO
				.find(transaccionBancaria.getCuentaBancaria()
		...
		if (transaccionBancaria.getEstado() == true
			...
			transaccionBancaria.setEstado(false);
			actualizarSaldoCaja(transaccionBancaria.getMonto().abs(),
					transaccionBancaria.getMoneda().getIdMoneda());
		} else
			throw new RollbackFailureException(
					"Error al Extornar Transacci&oacute;n: Transacci&oacute;n o cuenta bancaria no activa");
	}

	private void extornarTransaccionCuentaAporte(BigInteger idTransaccion)
			throws RollbackFailureException {
		TransaccionCuentaAporte transaccionCuentaAporte = transaccionCuentaAporteDAO
				.find(idTransaccion);
		CuentaAporte cuentaAporte = cuentaAporteDAO
				.find(transaccionCuentaAporte.getCuentaAporte()
						.getIdCuentaaporte());
		HistorialCaja historialCajaActivo = new HistorialCaja();
		for (HistorialCaja historialCaja : getCaja().getHistorialCajas()) {
			historialCajaActivo = historialCaja;
		...
		if (transaccionCuentaAporte.getEstado() == true
				&& cuentaAporte.getEstadoCuenta().equals(
						EstadoCuentaAporte.ACTIVO)
				&& transaccionCuentaAporte.getHistorialCaja()
						.getIdHistorialCaja() == historialCajaActivo
						.getIdHistorialCaja()) {
			Caja caja = this.getCaja();
			BigDecimal saldoActualBovedaCaja = new BigDecimal(0.00);
			Set<BovedaCaja> bovedasCajas = caja.getBovedaCajas();
			...
			if (saldoActualBovedaCaja.compareTo(transaccionCuentaAporte
					.getMonto()) != -1) {
				cuentaAporte.setSaldo(cuentaAporte.getSaldo().subtract(
						transaccionCuentaAporte.getMonto()));
				transaccionCuentaAporte.setEstado(false);
				actualizarSaldoCaja(transaccionCuentaAporte.getMonto().abs()
						.negate(), cuentaAporte.getMoneda().getIdMoneda());
			} else
				throw new RollbackFailureException(
						"Error al Extornar Transacci&oacute;n: Saldo insuficiente en caja");
		} else
	...
	private void extornarTransaccionCompraVenta(BigInteger idTransaccion)
			throws RollbackFailureException {
		TransaccionCompraVenta transaccionCompraVenta = transaccionCompraVentaDAO
				.find(idTransaccion);
		HistorialCaja historialCajaActivo = new HistorialCaja();
		for (HistorialCaja historialCaja : getCaja().getHistorialCajas()) {
			historialCajaActivo = historialCaja;
		...
		if (transaccionCompraVenta.getEstado() == true
				&& transaccionCompraVenta.getHistorialCaja()
						.getIdHistorialCaja() == historialCajaActivo
						.getIdHistorialCaja()) {
			Caja caja = this.getCaja();
			BigDecimal saldoActualBovedaCaja = new BigDecimal(0.00);
			...
			for (BovedaCaja bovedaCaja : bovedasCajas) {
				Moneda monedaBoveda = bovedaCaja.getBoveda().getMoneda();
				if (transaccionCompraVenta.getMonedaRecibida().equals(
						monedaBoveda)) {
					saldoActualBovedaCaja = bovedaCaja.getSaldo();
					break;
				}
			...
			if (saldoActualBovedaCaja.compareTo(transaccionCompraVenta
					.getMontoRecibido()) != -1) {
				actualizarSaldoCaja(transaccionCompraVenta.getMontoRecibido()
						.abs().negate(), transaccionCompraVenta
						.getMonedaRecibida().getIdMoneda());
				actualizarSaldoCaja(transaccionCompraVenta.getMontoEntregado()
						.abs(), transaccionCompraVenta.getMonedaEntregada()
						.getIdMoneda());
	...
	private void extornarTransferenciaBancaria(BigInteger idTransaccion)
			throws RollbackFailureException {
		TransferenciaBancaria transferenciaBancaria = transferenciaBancariaDAO
				.find(idTransaccion);
		throw new RollbackFailureException(
				"Todavia no es posible extornar las transferencias");
	}
	...
	public BigInteger[] crearCuentaBancariaPlazoFijoConDeposito(String codigo,
		...
		BigInteger idCuentaBancaria = cuentaBancariaService.create(
				TipoCuentaBancaria.PLAZO_FIJO, codigo, idMoneda, tasaInteres,
				tipoPersona, idPersona, new Integer(periodo), cantRetirantes,
				titulares, beneficiarios);
		CuentaBancaria cuentaBancaria = cuentaBancariaDAO
				.find(idCuentaBancaria);
		String numeroCuenta = cuentaBancaria.getNumeroCuenta();
		BigInteger idTransaccion = crearDepositoBancario(numeroCuenta, monto,
				"APERTURA CUENTA BANCARIA PLAZO FIJO");

		return new BigInteger[] { idCuentaBancaria, idTransaccion };
	}

	@Override
	public BigInteger cancelarCuentaBancariaConRetiro(BigInteger id)
			throws RollbackFailureException {
		...
		if (!cuentaBancaria.getEstado().equals(EstadoCuentaBancaria.ACTIVO))
			throw new RollbackFailureException(
					"La cuenta no esta activa, no se puede cancelar");
		if (cuentaBancaria.getTipoCuentaBancaria().equals(
				TipoCuentaBancaria.PLAZO_FIJO)) {
			if (cuentaBancaria.getFechaCierre().compareTo(
		...
		BigInteger idTransaccion = crearRetiroBancario(numeroCuenta, monto,
				referencia);
	...
	@Override
	public BigInteger cancelarSocioConRetiro(BigInteger idSocio)
			throws RollbackFailureException {
		Socio socio = socioDAO.find(idSocio);
		if (socio == null)
			throw new RollbackFailureException("Socio no encontrado");
	...
	@Override
	@AllowedTo(Permission.ABIERTO)
	public BigInteger crearPendiente(BigInteger idBoveda, BigDecimal monto,
		...
		if (bovedaCajaTransaccion == null)
			throw new RollbackFailureException(
					"La caja y la boveda seleccionados no estan relacionados");
		...
		pendienteCaja
				.setTipoPendiente(monto.compareTo(BigDecimal.ZERO) >= 1 ? TipoPendiente.FALTANTE
						: TipoPendiente.SOBRANTE);
		pendienteCaja.setObservacion(observacion);
		pendienteCaja.setTrabajador(trabajador.getPersonaNatural()
				.getApellidoPaterno()
				+ " "
				+ trabajador.getPersonaNatural().getApellidoMaterno()
				+ ", "
	...
	@Override
	@AllowedTo(Permission.ABIERTO)
	public BigInteger crearTransaccionBovedaCaja(BigInteger idBoveda,
		...
		QueryParameter queryParameter1 = QueryParameter.with("idboveda",
				idBoveda);
		List<HistorialBoveda> listHistBovedas = historialBovedaDAO
				.findByNamedQuery(HistorialBoveda.findByHistorialActivo,
						queryParameter1.parameters());
		if (listHistBovedas.size() > 1) {
			throw new RollbackFailureException(
					"La boveda tiene mas de un historial activo");
		} else {
		...
		if (historialBoveda == null)
			throw new RollbackFailureException(
					"Boveda no tiene historiales activos");
		...
		Set<DetalleHistorialBoveda> detHistBoveda = historialBoveda
	...
	@Override
	@AllowedTo(Permission.ABIERTO)
	public BigInteger crearTransaccionCajaCaja(BigInteger idCajadestino,
			BigInteger idMoneda, BigDecimal monto, String observacion)
			throws RollbackFailureException {
	...
	public void cancelarTransaccionBovedaCaja(BigInteger idTransaccionBovedaCaja)
		...
		if (transaccionBovedaCaja.getEstadoConfirmacion() == true)
			throw new RollbackFailureException(
					"Transaccion ya fue CONFIRMADA, no se puede cancelar");
		if (transaccionBovedaCaja.getEstadoSolicitud() == false)
			throw new RollbackFailureException(
					"Transaccion ya fue CANCELADA, no se puede cancelar nuevamente");
		if (!transaccionBovedaCaja.getOrigen().equals(
				TransaccionBovedaCajaOrigen.CAJA))
			throw new RollbackFailureException(
					"No se puede cancelar una transaccion solicitada por una boveda");
	...
	@Override
	@AllowedTo(Permission.ABIERTO)
	public void confirmarTransaccionBovedaCaja(
			BigInteger idTransaccionBovedaCaja) throws RollbackFailureException {
		TransaccionBovedaCaja transaccionBovedaCaja = transaccionBovedaCajaDAO
				.find(idTransaccionBovedaCaja);
		if (transaccionBovedaCaja == null)
			throw new RollbackFailureException("Transaccion no encontrada");
		if (transaccionBovedaCaja.getEstadoSolicitud() == false)
			throw new RollbackFailureException(
					"Transaccion ya fue CANCELADA, no se puede confirmar");
		if (transaccionBovedaCaja.getEstadoConfirmacion() == true)
			throw new RollbackFailureException(
		...
		if (!transaccionBovedaCaja.getOrigen().equals(
			...
			throw new RollbackFailureException(
					"No se puede confirmar una transaccion solicitada por una caja");
		TransaccionBovedaCajaView view = transaccionBovedaCajaViewDAO
				.find(idTransaccionBovedaCaja);
		BigDecimal monto = view.getMonto();
		Moneda moneda = transaccionBovedaCaja.getHistorialBoveda().getBoveda()
				.getMoneda();
	...
	@Override
	public BigInteger crearTransaccionBancaria(String numeroCuenta,
	...
	@Override
	public void cancelarTransaccionCajaCaja(BigInteger idTransaccionCajaCaja)
			throws RollbackFailureException {
		// TODO Auto-generated method stub
