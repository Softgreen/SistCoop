...
public class EntidadBeanDAO implements DAO<Object, Entidad> {

	@PersistenceContext
	private EntityManager em;

	public Entidad create(Entidad t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Entidad t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Entidad find(Object id) {
		return this.em.find(Entidad.class, id);
	}

	public Entidad update(Entidad t) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Entidad> rt = cq.from(Entidad.class);
	...
	public List<Entidad> findByNamedQuery(String namedQueryName) {
	...
	public List<Entidad> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Entidad> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
