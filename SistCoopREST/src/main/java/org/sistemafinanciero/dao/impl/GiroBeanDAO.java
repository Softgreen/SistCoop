...
public class GiroBeanDAO implements DAO<Object, Giro> {

	@PersistenceContext
	private EntityManager em;

	public Giro create(Giro t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Giro t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Giro find(Object id) {
		return this.em.find(Giro.class, id);
	}

	public Giro update(Giro t) {
	...
	public List<Giro> findAll() {
		...
		cq.select(cq.from(Giro.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<Giro> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Giro> rt = cq.from(Giro.class);
	...
	public List<Giro> findByNamedQuery(String namedQueryName) {
	...
	public List<Giro> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Giro> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
