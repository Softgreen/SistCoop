...
public class DebeHaberBeanDAO implements DAO<Object, DebeHaber> {

    @PersistenceContext
    private EntityManager em;

    public DebeHaber create(DebeHaber t) {
        this.em.persist(t);
        return t;
    }

    public void delete(DebeHaber t) {
        t = this.em.merge(t);
        this.em.remove(t);
    }

    public DebeHaber find(Object id) {
        return this.em.find(DebeHaber.class, id);
    }

    public DebeHaber update(DebeHaber t) {
        return this.em.merge(t);
    }

    public List<DebeHaber> findAll() {
        List<DebeHaber> list = null;
        CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        cq.select(cq.from(DebeHaber.class));
        list = this.em.createQuery(cq).getResultList();
        return list;
    }

    public List<DebeHaber> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<DebeHaber> rt = cq.from(DebeHaber.class);
    ...
    public List<DebeHaber> findByNamedQuery(String namedQueryName) {
        return this.em.createNamedQuery(namedQueryName).getResultList();
    }

    public List<DebeHaber> findByNamedQuery(String namedQueryName, Map<String, Object> parameters) {
        return findByNamedQuery(namedQueryName, parameters, 0);
    }

    public List<DebeHaber> findByNamedQuery(String queryName, int resultLimit) {
