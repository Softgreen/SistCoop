...
public class ChequeraBeanDAO implements DAO<Object, Chequera> {

	@PersistenceContext
	private EntityManager em;

	public Chequera create(Chequera t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Chequera t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Chequera find(Object id) {
		return this.em.find(Chequera.class, id);
	}

	public Chequera update(Chequera t) {
		return this.em.merge(t);
	}

	public List<Chequera> findAll() {
		List<Chequera> list = null;
		CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
		cq.select(cq.from(Chequera.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<Chequera> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Chequera> rt = cq.from(Chequera.class);
	...
	public List<Chequera> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Chequera> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
