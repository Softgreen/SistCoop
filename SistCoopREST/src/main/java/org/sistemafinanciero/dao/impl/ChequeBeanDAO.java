...
public class ChequeBeanDAO implements DAO<Object, Cheque> {

	@PersistenceContext
	private EntityManager em;

	public Cheque create(Cheque t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Cheque t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Cheque find(Object id) {
		return this.em.find(Cheque.class, id);
	}

	public Cheque update(Cheque t) {
	...
	public List<Cheque> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Cheque> rt = cq.from(Cheque.class);
	...
	public List<Cheque> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Cheque> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
