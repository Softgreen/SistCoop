...
public class SucursalBeanDAO implements DAO<Object, Sucursal> {
	...
	public Sucursal find(Object id) {
		return this.em.find(Sucursal.class, id);
	}
	...
	public List<Sucursal> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Sucursal> rt = cq.from(Sucursal.class);
	...
	public List<Sucursal> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Sucursal> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
